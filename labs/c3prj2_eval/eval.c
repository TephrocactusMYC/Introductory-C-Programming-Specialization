...
suit_t flush_suit(deck_t * hand) {
  size_t SPADES_TIMES=0;
    size_t HEARTS_TIMES=0;
    size_t DIAMONDS_TIMES=0;
    size_t CLUBS_TIMES=0;
...
int compare_hands(deck_t * hand1, deck_t * hand2) {
  ...
  if (result1.ranking != result2.ranking) {
    return result2.ranking - result1.ranking;
  }
  ...
  for (int i = 0; i < 5; i++) {
    if (result1.cards[i]->value == result2.cards[i]->value) {
      ...
      return result1.cards[i]->value - result2.cards[i]->value;
    }
  }
  return 0;
}
...
unsigned * get_match_counts(deck_t * hand){
    unsigned *p=malloc((hand->n_cards)*sizeof(*p));
    for(int i=0;i<hand->n_cards;i++){
	p[i]=0;
